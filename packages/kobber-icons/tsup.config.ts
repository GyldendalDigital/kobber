...
`;
  ...
  const makeStories = (symbols: NodeListOf<SVGSymbolElement>) => {
    ...
    symbols.forEach(symbol => {
      ...
      iconGalleryMainImportsString += `${iconName.unprefixedCapitalized}, `;
      iconGalleryString += `\t<IconItem name="${iconName.prefixedCapitalized} - <${iconName.prefixed} />">\n\t\t<${iconName.prefixed} class="kobber-theme-default" />\n\t</IconItem>\n`;
...
`;
    ...
    iconGalleryMainImportsString += `} from "../index.web-components";\n\n`;
    iconGalleryString += `\n</IconGallery>\n`;
    fs.writeFileSync(
