...
import type RadioInput from "./RadioInput";
import { buttonGroupName, inputName, horizontalClassName } from "./Radio.core";
...
@customElement(buttonGroupName)
export default class RadioGroup extends ShoelaceElement implements ShoelaceFormControl {
  ...
  private getAllRadios() {
    return [...this.querySelectorAll<RadioInput>(inputName)];
  }

  private handleRadioClick(event: MouseEvent) {
    const target = (event.target as HTMLElement).closest<RadioInput>(inputName)!;
  ...
  private syncRadios() {
    if (customElements.get(inputName)) {
    ...
    if (customElements.get(inputName)) {
      ...
      customElements.whenDefined(inputName).then(() => this.syncRadios());
    }
  }
  ...
  render() {
    ...
    return html`
      ...
      >
        ...
        <div part="form-control-input" class="${this.direction === "horizontal" ? horizontalClassName : ""}">
          <div class="visually-hidden">
            <!-- Bruker et usynlig tekstfelt for å plassere browservaliderings på ønsket plass. Lag noe bedre. -->
        ...
        <div part="form-control-help-text" id="help-text" aria-hidden=${hasHelpText ? "false" : "true"}>
