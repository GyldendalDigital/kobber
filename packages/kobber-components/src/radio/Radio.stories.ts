...
import { buttonVariants, ButtonProps, inputName } from "./Radio.core";
    ...
    return `
      ...
      <div class="wrapper-theme">
        ...
        <p slot="label" class="${!args.showLabel ? "visually-hidden" : ""}">
          ...
          <kobber-radio-input value="hardcover" group="format" ${args.link ? `href="#format-innbundet"` : ""}
             variant="${args.variant}"><div>Innbundet – <em style="text-wrap: nowrap">kr 2 339,-</em></div></kobber-radio-input
          >
          <kobber-radio-input value="pocket" group="format" ${args.link ? `href="#format-pocket"` : ""} disabled
             variant="${args.variant}"><div>Pocket – <em style="text-wrap: nowrap">kr 339,-</em><p class="alert">Utsolgt</p></div></kobber-radio-input
          >
          <kobber-radio-input value="ebook" group="format" ${args.link ? `href="#format-ebok"` : ""}
          ...
          >
          <kobber-radio-input value="audiobook" group="format" ${args.link ? `href="#format-lydbok"` : ""}
...
export const SmartExerciseExample: Story = {
  render: args => {
    return `
      ...
      <div class="wrapper-theme">
        <kobber-radio-group current-value="DontUnderstand">
        <p slot="label">
          Spør lærer
        </p>
        
          <kobber-radio-input value="DontUnderstand" group="help-request-form" variant="${args.variant}">Jeg forstår ikke oppgaven</kobber-radio-input>
          <kobber-radio-input value="SomethingWrong" group="help-request-form" variant="${args.variant}">Jeg tror det er noe feil med oppgaven</kobber-radio-input>
          <kobber-radio-input value="Other" group="help-request-form" variant="${args.variant}">Annet</kobber-radio-input>
    ...
    return `
      ...
      <div class="wrapper-theme">
        ...
        <p slot="label">Målform</p>
          <kobber-radio-input value="no-bm" group="language" variant="${args.variant}">Bokmål</kobber-radio-input>
          <kobber-radio-input value="no-nn" group="language" variant="${args.variant}">Nynorsk</kobber-radio-input>
        </kobber-radio-group>

        <kobber-radio-group direction="horizontal" current-value="level11-13">
        <p slot="label">Trinn</p>
          <kobber-radio-input value="level1-7" group="level" variant="${args.variant}">1.–7. trinn</kobber-radio-input>
          <kobber-radio-input value="level8-10" group="level" variant="${args.variant}">8.–10. trinn</kobber-radio-input>
          <kobber-radio-input value="level11-13" group="level" variant="${args.variant}">VG1–VG3</kobber-radio-input>
        </kobber-radio-group>
      </div>
    `;
  },
