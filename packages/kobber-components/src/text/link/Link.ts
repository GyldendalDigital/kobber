...
import { isButton, isExternalLink, linkClassNames, LinkExtendedColor, linkName, LinkProps } from "./Link.core";
...
export class Link extends LitElement implements LinkProps {
  ...
  render() {
    ...
    return html`
      <${tag} class="${linkClassNames({ extendedColor: this.extendedColor }).join(" ")} ${this.className}" href=${ifDefined(this.href)} target=${ifDefined(target)}>
        <slot></slot>
        ${
          isExternal
