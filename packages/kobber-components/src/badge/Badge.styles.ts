...
const getConcreteThemeMainVariantStyles = () => {
  return css`
    ${unsafeCSS(`
    --background-color: var(${unsafeCSS(component.badge.background.color.concrete.main)});
    --color: var(${unsafeCSS(component.badge.text.color.concrete.main)});
      `)}
  `;
};
...
const getNotConcreteThemeVariantStyles = (theme: "aubergine" | "rettsdata", variant: BadgeVariant) => {
  return css`
    ${unsafeCSS(`
      --background-color: var(${unsafeCSS(component.badge.background.color[theme][variant])});
      --color: var(${unsafeCSS(component.badge.text.color[theme][variant])});
      `)}
  `;
};
...
const spacingStyles = (size: BadgeSize) => {
  const containerStyles = component.badge.container;
  let paddingStyles = "0";
