...
const drawBreakPoints = () => {
  ...
  Object.entries(layout.viewportWidth).forEach(([key, value]) => {
    ...
    const max = "max" in value ? value.max : 0
    const x = gap + min
    const y = gap
    context.fillText(`${key} (${min}px - ${max ? `${max}px` : "Infinity"})`, x, y)
...
export const drawSlope = (minLabel: string, maxLabel: string, min: number, max: number) => {
  setFont()
  const x = 0
  const y = 100
